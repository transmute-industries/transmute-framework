...
describe("TransmuteIpfs", () => {
  ...
  describe("addFromFs(folderPath, options)", () => {
    ...
    it("should add folder to ipfs and return the result", () => {
      return TransmuteIpfs.addFromFs(
        "./src/TransmuteIpfs/mock/demo"
      ).then(res => {
  ...
  describe("addFromURL(url)", () => {
    ...
    it("should add folder to ipfs and return the result", () => {
      return TransmuteIpfs.addFromURL(
        "https://vignette3.wikia.nocookie.net/nyancat/images/7/7c/Nyan_gary.gif"
      ).then(res => {
        assert.equal(
          res[0].path,
          "QmSAKHa2JdKrhmBKrqWrBAtS7ACwofiRAEWzYNbXdssBEe"
        );
        assert.equal(
          res[0].hash,
          "QmSAKHa2JdKrhmBKrqWrBAtS7ACwofiRAEWzYNbXdssBEe"
  ...
  describe("writeObject(obj)", () => {
    ...
    it("should add json object to ipfs and return the path", () => {
      ...
      return TransmuteIpfs.writeObject(obj).then(path => {
  ...
  describe("readObject(path)", () => {
    ...
    it("should add json object to ipfs and return the path", () => {
      return TransmuteIpfs.readObject(
