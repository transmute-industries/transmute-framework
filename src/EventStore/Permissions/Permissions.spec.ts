...
describe("Permissions", () => {
  ...
  describe(".setAddressRole", () => {
    it("owner can make account 1 an admin", async () => {
      let { tx, events } = await Permissions.setAddressRole(
        acc,
        fromAddress,
        account_addresses[1],
        "admin"
      ...
      assert.equal(
        events[0].payload[account_addresses[1]],
        "admin",
  ...
  describe(".setGrant", () => {
    it("owner can grant admin role create:any eventstore", async () => {
      let { tx, events } = await Permissions.setGrant(
        acc,
        fromAddress,
        "admin",
        "eventstore",
  ...
  describe(".canRoleActionResource", () => {
    it("owner can check if role is granted action on resource", async () => {
      let granted = await Permissions.canRoleActionResource(
        acc,
        fromAddress,
        "admin",
        "create:any",
  ...
  describe(".getPermissionsReadModel", () => {
    it("return grants as object", async () => {
      let readModel = await TransmuteFramework.Permissions.getPermissionsReadModel(
        acc,
        fromAddress
      )
