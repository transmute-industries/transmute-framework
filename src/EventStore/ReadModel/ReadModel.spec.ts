...
describe("ReadModel", () => {
  ...
  describe(".readModelGenerator", () => {
    it("should return the initial read model when passed an empty array", () => {
      ...
      let updatedReadModel = TransmuteFramework.ReadModel.readModelGenerator(
        factoryReadModel,
        factoryReducer,
        []
      );
      assert(
        _.isEqual(factoryReadModel, updatedReadModel),
        "expected no changes from application of empty event array"
      );
    });

    it("should return an updated read model when passed a non-empty event array", async () => {
      let events = await TransmuteFramework.EventStore.readFSAs(
        factory,
        fromAddress,
        0
      );
      ...
      assert.equal(
        updatedReadModel.lastEvent,
        0,
        "expected event 0 to have been applied to the read model"
      );
    });
  ...
  describe(".maybeSyncReadModel", () => {
    ...
    it("should return the same read model if it is up to date", async () => {
      ...
      assert(
        _.isEqual(updatedReadModel1, updatedReadModel2),
        "expected no changes when no new events have been saved"
      );
    });
    ...
    it("should return an updated read model when new events have been saved", async () => {
      ...
      let { tx, events } = await TransmuteFramework.Factory.createEventStore(
        factory,
        fromAddress
      );
      ...
      assert.equal(
        updatedReadModel.lastEvent,
        events[0].meta.id,
        "expected 1 more event to have been applied"
      );
    });
  });
