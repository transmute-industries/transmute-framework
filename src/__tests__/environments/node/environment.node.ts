...
const FRAMEWORK_ENV_PATH = path.join(__dirname, '../../../../../secrets/environment.secret.env')
...
const GOOGLE_PROJECT_SERVICE_ACCOUNT_ABS_PATH =
  FRAMEWORK_ENV === 'NODE_LOCAL'
    ? process.env.GOOGLE_PROJECT_SERVICE_ACCOUNT_ABS_PATH
    : config.dapp.google_project_service_account_abs_path
const GOOGLE_PROJECT_FIREBASE_CONFIG_ABS_PATH =
  FRAMEWORK_ENV === 'NODE_LOCAL'
    ? process.env.GOOGLE_PROJECT_FIREBASE_CONFIG_ABS_PATH
    : config.dapp.google_project_firebase_config_abs_path
const GOOGLE_PROJECT_NAME =
  FRAMEWORK_ENV === 'NODE_LOCAL' ? process.env.GOOGLE_PROJECT_NAME : config.dapp.google_project_name
const TRANSMUTE_API_BASE_URL =
  FRAMEWORK_ENV === 'NODE_LOCAL' ? process.env.TRANSMUTE_API_BASE_URL : config.dapp.transmute_api_base_url

if (FRAMEWORK_ENV === 'NODE_LOCAL') {
  admin.initializeApp({
    credential: admin.credential.cert(require(GOOGLE_PROJECT_SERVICE_ACCOUNT_ABS_PATH)),
  })
}
