...
describe.only('EventStore', () => {
  ...
  contract('Factory', function (accounts) {
    ...
    it('createEventStore.call', async () => {
      firstEventStoreAddress = await factory.createEventStore.call({ from: eventStoreCreator })
    })

    it('createEventStore', async () => {
      let _tx = await factory.createEventStore(
        { from: eventStoreCreator, gas: 2000000 })
      let event = _tx.logs[0].args
      console.log(event)
      let eventType = toAscii(event.Type)
      assert.equal(eventType, 'ES_CREATED', 'expect first event to be Type ES_CREATED')
